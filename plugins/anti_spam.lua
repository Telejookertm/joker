    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "اسپم کردن در این گروه مجاز نیست\n@"..username.."["..msg.from.id.."]\nوضعیت: کاربر اخراج شد")
		else
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "کاربر [ "..name.." ]"..msg.from.id.." مسدود جهانی (spamming)")
		  send_large_msg("channel#id"..msg.to.id, "کاربر [ "..name.." ]"..msg.from.id.." مسدود جهان شد (spamming)")
          local GBan_log = 'GBan_log'
		  local GBan_log =  data[tostring(GBan_log)]
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "کاربر [ "..name.." ] ( @"..username.." )"..msg.from.id.." مسدود جهان شد ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] (spamming)"
